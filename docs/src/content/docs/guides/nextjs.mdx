---
title: Next.js
description: Connect explicit DI scopes to App Router server and client boundaries.
---
...
For nested Server Components, repeated calls to `getRequestContainer()` resolve
through the same request container during the current render.
...
The same helper fits Server Actions, tests, jobs, and custom server entry points.

## Node Runtime ALS-First Entry Points

For Node runtime Next.js entry points where deep async code should call
...
export async function createPost(input: FormData) {
  ...
  return runWithRequestContainer({
    ...
    run: async () => {
      ...
      revalidateTag(`tenant:${tenantId}:posts`, "max");
    },
  });
}
```
...
For APIs such as `after`, capture the primitive values the deferred
...
The original async scope is inactive after disposal, and
`getRequestContainer()` rejects access to it. Re-enter a new scope, or hoist the
work to a queue/background job that receives plain values.

The Route Handler and Server Action examples are compiled alongside the docs:
