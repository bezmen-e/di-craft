...
That distinction matters in multi-tenant or resource-heavy entry points:
...
The full Next.js Node runtime example shows the entry point wrapper and deep
...
Next.js post-action work scheduled with `after()` may inherit the runtime async
context, but di-craft marks that request scope inactive after disposal. Calling
`getRequestContainer()` from the deferred callback throws
`NodeRequestScopeError`. Re-enter a new scope or pass plain values to that work.
